...
function jmy_web_url_friendly(a){return a.toLowerCase().replace(/[^a-z0-9]+/g, "-").replace(/^-+|-+$/g, "-").replace(/^-+|-+$/g, '')};
	...
	jQuery(function ($) { 
		...
		$(".jmy_select_guardar").on('click',function(){
			...
			if(g.nombre!=''&&g.nombre!=undefined){
				g.value = (g.value!=''&&g.value!=undefined)?jmy_web_url_friendly(g.value):jmy_web_url_friendly(g.nombre);
				console.log(g);
